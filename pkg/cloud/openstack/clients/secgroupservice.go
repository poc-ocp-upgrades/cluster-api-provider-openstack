...
var defaultRules = []openstackconfigv1.SecurityGroupRule{{Direction: "egress", EtherType: "IPv4", PortRangeMin: 0, PortRangeMax: 0, Protocol: "", RemoteIPPrefix: ""}, {Direction: "egress", EtherType: "IPv6", PortRangeMin: 0, PortRangeMax: 0, Protocol: "", RemoteIPPrefix: ""}}

type SecGroupService struct{ client *gophercloud.ServiceClient }
...
func (s *SecGroupService) Reconcile(clusterName string, desired openstackconfigv1.OpenstackClusterProviderSpec, status *openstackconfigv1.OpenstackClusterProviderStatus) error {
	...
	desiredSecGroups := map[string]openstackconfigv1.SecurityGroup{"controlplane": s.generateControlPlaneGroup(clusterName), "global": s.generateGlobalGroup(clusterName)}
...
func (s *SecGroupService) createRule(r openstackconfigv1.SecurityGroupRule) (openstackconfigv1.SecurityGroupRule, error) {
	...
	createOpts := rules.CreateOpts{Direction: dir, PortRangeMin: r.PortRangeMin, PortRangeMax: r.PortRangeMax, Protocol: proto, EtherType: etherType, RemoteGroupID: r.RemoteGroupID, RemoteIPPrefix: r.RemoteIPPrefix, SecGroupID: r.SecurityGroupID}
