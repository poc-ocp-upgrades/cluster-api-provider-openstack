...
type Instance struct{ servers.Server }
...
func GetCloudFromSecret(kubeClient kubernetes.Interface, namespace string, secretName string, cloudName string) (clientconfig.Cloud, error) {
	...
	if !ok {
		return emptyCloud, fmt.Errorf("OpenStack credentials secret %v did not contain key %v", secretName, CloudsSecretKey)
	}
...
func NewInstanceServiceFromCloud(cloud clientconfig.Cloud) (*InstanceService, error) {
	...
	identityClient, err := openstack.NewIdentityV3(provider, gophercloud.EndpointOpts{Region: ""})
	if err != nil {
		return nil, fmt.Errorf("Create identityClient err: %v", err)
	}
	serverClient, err := openstack.NewComputeV2(provider, gophercloud.EndpointOpts{Region: clientOpts.RegionName})
	if err != nil {
		return nil, fmt.Errorf("Create serviceClient err: %v", err)
	}
	networkingClient, err := openstack.NewNetworkV2(provider, gophercloud.EndpointOpts{Region: clientOpts.RegionName})
	if err != nil {
		return nil, fmt.Errorf("Create networkingClient err: %v", err)
	}
	imagesClient, err := openstack.NewImageServiceV2(provider, gophercloud.EndpointOpts{Region: clientOpts.RegionName})
	if err != nil {
		return nil, fmt.Errorf("Create ImageClient err: %v", err)
	}
	return &InstanceService{provider: provider, identityClient: identityClient, computeClient: serverClient, networkClient: networkingClient, imagesClient: imagesClient}, nil
}
...
func (is *InstanceService) InstanceCreate(clusterName string, name string, clusterSpec *openstackconfigv1.OpenstackClusterProviderSpec, config *openstackconfigv1.OpenstackProviderSpec, cmd string, keyName string) (instance *Instance, err error) {
	...
	machineTags := []string{"cluster-api-provider-openstack", clusterName}
	...
	for _, net := range config.Networks {
		...
		for _, netID := range ids {
			if net.Subnets == nil {
				nets = append(nets, ServerNetwork{networkID: netID})
			}
			for _, snet := range net.Subnets {
				...
				for _, snet := range snets {
					nets = append(nets, ServerNetwork{networkID: snet.NetworkID, subnetID: snet.ID})
				}
			}
		}
	}
	...
	for _, net := range nets {
		...
		allPages, err := ports.List(is.networkClient, ports.ListOpts{Name: name, NetworkID: net.networkID}).AllPages()
		if err != nil {
		...
		_, err = attributestags.ReplaceAll(is.networkClient, "ports", port.ID, attributestags.ReplaceAllOpts{Tags: machineTags}).Extract()
		if err != nil {
			return nil, fmt.Errorf("Tagging port for server err: %v", err)
		}
		ports_list = append(ports_list, servers.Network{Port: port.ID})
		if config.Trunk == true {
			allPages, err := trunks.List(is.networkClient, trunks.ListOpts{Name: name, PortID: port.ID}).AllPages()
			if err != nil {
			...
			if len(trunkList) == 0 {
				trunkCreateOpts := trunks.CreateOpts{Name: name, PortID: port.ID}
			...
			_, err = attributestags.ReplaceAll(is.networkClient, "trunks", trunk.ID, attributestags.ReplaceAllOpts{Tags: machineTags}).Extract()
			if err != nil {
	...
	serverCreateOpts := servers.CreateOpts{Name: name, ImageRef: imageID, FlavorName: config.Flavor, AvailabilityZone: config.AvailabilityZone, Networks: ports_list, UserData: []byte(userData), SecurityGroups: securityGroups, ServiceClient: is.computeClient, Tags: serverTags, Metadata: config.ServerMetadata, ConfigDrive: config.ConfigDrive}
	if config.RootVolume != nil && config.RootVolume.Size != 0 {
		var blocks []bootfromvolume.BlockDevice
		block := bootfromvolume.BlockDevice{SourceType: bootfromvolume.SourceType(config.RootVolume.SourceType), BootIndex: 0, UUID: config.RootVolume.SourceUUID, DeleteOnTermination: true, DestinationType: bootfromvolume.DestinationVolume, VolumeSize: config.RootVolume.Size, DeviceType: config.RootVolume.DeviceType}
		blocks = append(blocks, block)
		createOpts = bootfromvolume.CreateOptsExt{CreateOptsBuilder: createOpts, BlockDevice: blocks}
	}
	server, err := servers.Create(is.computeClient, keypairs.CreateOptsExt{CreateOptsBuilder: serverCreateOpts, KeyName: keyName}).Extract()
	if err != nil {
		return nil, fmt.Errorf("Create new server err: %v", err)
	}
...
func (is *InstanceService) InstanceDelete(id string) error {
	...
	for _, port := range instanceInterfaces {
		...
		if trunkSupport {
			listOpts := trunks.ListOpts{PortID: port.PortID}
