...
func (s *NetworkService) reconcileNetwork(clusterName, networkName string, desired openstackconfigv1.OpenstackClusterProviderSpec) (openstackconfigv1.Network, error) {
	...
	if res.ID != "" {
		return openstackconfigv1.Network{ID: res.ID, Name: res.Name}, nil
	}
	opts := networks.CreateOpts{AdminStateUp: gophercloud.Enabled, Name: networkName}
	network, err := networks.Create(s.client, opts).Extract()
	if err != nil {
		return emptyNetwork, err
	}
	_, err = attributestags.ReplaceAll(s.client, "networks", network.ID, attributestags.ReplaceAllOpts{Tags: []string{"cluster-api-provider-openstack", clusterName}}).Extract()
	if err != nil {
		return emptyNetwork, err
	}
...
func (s *NetworkService) reconcileSubnets(clusterName, name string, desired openstackconfigv1.OpenstackClusterProviderSpec, network openstackconfigv1.Network) (openstackconfigv1.Subnet, error) {
	...
	allPages, err := subnets.List(s.client, subnets.ListOpts{NetworkID: network.ID, CIDR: desired.NodeCIDR}).AllPages()
	if err != nil {
	...
	if len(subnetList) > 1 {
		...
		opts := subnets.CreateOpts{NetworkID: network.ID, Name: name, IPVersion: 4, CIDR: desired.NodeCIDR, DNSNameservers: desired.DNSNameservers}
		newSubnet, err := subnets.Create(s.client, opts).Extract()
		if err != nil {
			return emptySubnet, err
		}
		observedSubnet = openstackconfigv1.Subnet{ID: newSubnet.ID, Name: newSubnet.Name, CIDR: newSubnet.CIDR}
	} else if len(subnetList) == 1 {
		observedSubnet = openstackconfigv1.Subnet{ID: subnetList[0].ID, Name: subnetList[0].Name, CIDR: subnetList[0].CIDR}
	}
	_, err = attributestags.ReplaceAll(s.client, "subnets", observedSubnet.ID, attributestags.ReplaceAllOpts{Tags: []string{"cluster-api-provider-openstack", clusterName}}).Extract()
	if err != nil {
		return emptySubnet, err
	}
...
func (s *NetworkService) reconcileRouter(clusterName, name string, desired openstackconfigv1.OpenstackClusterProviderSpec, network openstackconfigv1.Network) (openstackconfigv1.Router, error) {
	...
	allPages, err := routers.List(s.client, routers.ListOpts{Name: name}).AllPages()
	if err != nil {
	...
	if len(routerList) == 0 {
		opts := routers.CreateOpts{Name: name, GatewayInfo: &routers.GatewayInfo{NetworkID: desired.ExternalNetworkID}}
	...
	observedRouter := openstackconfigv1.Router{Name: router.Name, ID: router.ID}
...
INTERFACE_LOOP:
	...
	if createInterface {
		...
		iface, err := routers.AddInterface(s.client, router.ID, routers.AddInterfaceOpts{SubnetID: network.Subnet.ID}).Extract()
		if err != nil {
			return observedRouter, fmt.Errorf("unable to create router interface: %v", err)
		}
		klog.V(4).Infof("Created RouterInterface: %v", iface)
	}
	_, err = attributestags.ReplaceAll(s.client, "routers", observedRouter.ID, attributestags.ReplaceAllOpts{Tags: []string{"cluster-api-provider-openstack", clusterName}}).Extract()
	if err != nil {
		return emptyRouter, err
	}
